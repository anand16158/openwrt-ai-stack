				   ...
				   const struct flow_entry *entry)
{
	...
	blobmsg_add_string(&b, "class", classifier_class_name(entry->classification));
	blobmsg_add_u32(&b, "class_id", entry->classification);
	blobmsg_add_u32(&b, "confidence", (uint32_t)(entry->confidence * 100));
	...
	ubus_send_event(ctx->ubus, "traffic-classifier.classify", b.head);
	blob_buf_free(&b);
}
			    ...
			    const struct anomaly_event *ev)
{
	...
	snprintf(mac_str, sizeof(mac_str), "%02x:%02x:%02x:%02x:%02x:%02x",
		 ...
		 ev->mac[3], ev->mac[4], ev->mac[5]);
	blobmsg_add_string(&b, "mac", mac_str);
	blobmsg_add_string(&b, "type", anomaly_type_names[ev->type]);
	blobmsg_add_u64(&b, "timestamp", (uint64_t)ev->timestamp);
	blobmsg_add_string(&b, "detail", ev->detail);
	blobmsg_add_u32(&b, "severity", (uint32_t)(ev->severity * 100));

	ubus_send_event(ctx->ubus, "traffic-classifier.anomaly", b.head);
	blob_buf_free(&b);
}
